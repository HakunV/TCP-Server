...
public class MQTT_ProtocolHandler {
    ...
    private void handlePublish(String str) {
        ...
        int propertyLength = Integer.parseInt(str.substring(pointer, pointer+1*byteSize), 16);
        pointer = pointer+1*byteSize;

        if (propertyLength > 0) {
            String props = str.substring(pointer, pointer+propertyLength*byteSize);
            pointer = pointer+propertyLength*byteSize;
        }

        // Payload

        String payload = hexToString(str.substring(pointer, str.length()-1));
